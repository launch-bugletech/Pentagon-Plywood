# Pentagon Plywood Homepage Handoff
...
The standalone homepage consists of `index.html`, `style.css`, and `script.js`. Keep them in one directory with `favicon.svg` and `og.png`. Asset paths are relative, so the homepage can be opened directly by double-clicking `index.html` or served through any local static server.
