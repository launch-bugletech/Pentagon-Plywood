  ...
  return (
    <div className="applications-page">
      ...
      <section className="application-material-schedule" id="material-planning">
        <div className="container application-schedule-grid"><div><Heading light eyebrow="Build a Material Schedule" title="A Complete Project Does Not Have to Use One Board Everywhere." copy="A kitchen, wardrobe or commercial interior can combine materials according to each component’s function. The final schedule should be approved by the designer, fabricator or responsible project professional." /><a className="btn btn-accent" href={CONTACT_SECTIONS.form}>Share My Complete Material List <Icon.Arrow /></a></div><div className="application-stack">{[['Plywood', 'Selected structural and carcass components'], ['Blockboard', 'Suitable long panels and shutters'], ['MDF', 'Routed and painted fronts'], ['WPC', 'Selected moisture-conscious uses'], ['Flush doors', 'Interior room openings'], ['Laminates & veneers', 'Practical or premium visible finishes']].map(([title, copy], index) => <div key={title}><span>{String(index + 1).padStart(2, '0')}</span><strong>{title}</strong><small>{copy}</small></div>)}</div></div>
      </section>
      ...
      <section className="application-supply">
        <div className="container"><Heading light eyebrow="Manufactured and Sourced Materials" title="Bring the Application to Pentagon—not Just a Product Name." /><div className="application-supply-grid"><div><span>Manufactured by Pentagon</span>{['Plywood', 'Blockboard', 'Flush doors'].map((item) => <strong key={item}><Icon.Check />{item}</strong>)}</div><div><span>Sourced and supplied · subject to availability</span>{['MDF boards', 'WPC boards', 'Laminates', 'Natural & engineered veneers', 'Pre-laminated boards', 'Related decorative products'].map((item) => <strong key={item}><Icon.Check />{item}</strong>)}</div></div><p className="application-supply-note">Pentagon primarily supplies boards, doors and raw materials. Complete furniture manufacturing and turnkey interior execution are not standard services. Selected finished doors or customised products may be arranged on request.</p></div>
      </section>
