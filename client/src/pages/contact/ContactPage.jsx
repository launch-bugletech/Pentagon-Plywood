            ...
            {submitted ? (
              ...
              <>
                <div className="contact-form-head">
                  <div>
                    <div className="contact-eyebrow">Share Your Requirement</div>
                    <h2>{selectedType.title}</h2>
                  </div>
                  <span>01—03</span>
                </div>
                <p className="contact-form-intro">You do not need every technical specification. Tell us what you know, and our team can continue the discussion.</p>

                <div className="contact-form-grid">
                  <Field label="Enquiry type" required>
                    <select value={enquiryType} onChange={(event) => setEnquiryType(event.target.value)} required>
                      {enquiryTypes.map((item) => <option key={item.value} value={item.value}>{item.title}</option>)}
                    </select>
                  </Field>
                  <Field label="Product category" required>
                    <select value={productCategory} onChange={(event) => setProductCategory(event.target.value)} required>
                  ...
                  <Field label="Intended application">
                    <input type="text" placeholder="Furniture, kitchen, doors…" />
                  </Field>
                  <Field label="Estimated quantity">
                    <input type="text" placeholder={productCategory === 'Not Sure Yet' ? 'Optional' : 'e.g. 50'} />
                  </Field>
                ...
                {expanded && (
                  <div className="contact-form-more">
                    <div className="contact-form-step"><span>02</span><strong>Tell us about the requirement</strong></div>
                    <div className="contact-form-grid">
                      <Field label="Requirement details" required className="is-wide">
                        <textarea rows="4" placeholder="Tell us the application, size, thickness, finish or any other useful detail." required />
                      </Field>
                      <Field label="State" required><input type="text" required /></Field>
                      <Field label="Required-by date"><input type="date" /></Field>
                      <Field label="Upload a reference" className="is-wide">
                        <input type="file" accept=".pdf,.xls,.xlsx,.doc,.docx,image/*" />
                        <small>Optional: PDF, spreadsheet, drawing or reference image.</small>
                      </Field>
                    </div>
                    ...
                    {enquiryType === 'dealer' && (
                      <div className="contact-conditional">
                        ...
                        <div className="contact-form-grid">
                          <Field label="Business name" required><input type="text" required /></Field>
                          <Field label="City, district and state" required><input type="text" required /></Field>
                          <Field label="Current product categories"><input type="text" /></Field>
                          <Field label="Market or territory"><input type="text" /></Field>
                        </div>
                        <a className="contact-partner-link" href={ROUTES.dealers}>Read about Pentagon’s dealer and distributor process <ArrowIcon /></a>
                      </div>
                    )}
                    ...
                    {enquiryType === 'project' && (
                      <div className="contact-conditional">
                        ...
                        <div className="contact-form-grid">
                          <Field label="Project type" required><input type="text" required /></Field>
                          <Field label="Project location" required><input type="text" required /></Field>
                          <Field label="Approximate quantity" required><input type="text" required /></Field>
                          <Field label="Expected procurement date"><input type="date" /></Field>
                        </div>
                      </div>
                    )}
                    ...
                    {enquiryType === 'export' && (
                      <div className="contact-conditional">
                        ...
                        <div className="contact-form-grid">
                          <Field label="Company name" required><input type="text" required /></Field>
                          <Field label="Destination country and port" required><input type="text" required /></Field>
                          <Field label="Estimated order quantity" required><input type="text" required /></Field>
                          <Field label="Shipment preference"><input type="text" placeholder="If known" /></Field>
                        </div>
                      </div>
                    )}

                    <div className="contact-form-step"><span>03</span><strong>How can we reach you?</strong></div>
                    <div className="contact-form-grid">
                      ...
                      <Field label="Email address" required={enquiryType === 'export'}>
                        <input type="email" autoComplete="email" required={enquiryType === 'export'} />
                      </Field>
                      <Field label="Preferred contact">
                        <select defaultValue="Phone"><option>Phone</option><option>WhatsApp</option><option>Email</option></select>
                      </Field>
                    </div>
                ...
                <button className="btn btn-primary contact-submit" type="submit">
                  {expanded ? 'Send My Requirement' : 'Continue Your Requirement'} <ArrowIcon />
                </button>
                <p className="contact-form-footnote">Fields marked * are required. Your entered information remains in place if a field needs attention.</p>
              </>
            )}
          </form>
        </div>
          ...
          <div className="contact-type-grid">
            {enquiryTypes.map((item) => (
              <article className={item.value === enquiryType ? 'is-selected' : ''} key={item.value}>
                ...
                <h3>{item.title}</h3>
                <p>{item.copy}</p>
                <button type="button" onClick={() => selectEnquiry(item.value)}>{item.action} <ArrowIcon /></button>
              </article>
            ))}
          </div>
        </div>
